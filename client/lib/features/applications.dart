  ...
  Widget build(BuildContext context) => ResourceView(
    ...
    builder: (data) => ListView(
      ...
      children: [
        ...
        Text(
          'Gmail: ${friendly(object(data['gmail'])['state'] ?? data['email_sync_health'] ?? data['gmail_state'])}',
        ),
        const SizedBox(height: 12),
        const Text(
          'Waiting for response is derived after 24 hours without later employer activity.',
        ),
        const Divider(height: 32),
        ...objects(data['applications'] ?? data['items']).map(
          (a) => ListTile(
            title: Text('${label(a['company'])} · ${label(a['title'])}'),
            subtitle: Text(
              '${friendly(a['display_status'] ?? a['current_status'])} · Applied ${dateLabel(a['applied_at'])}',
            ),
