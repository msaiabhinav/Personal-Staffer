...
| JobSpy: Indeed, Google, Glassdoor, ZipRecruiter | Explicit site allowlist; independent source calls; pagination bounds; compatible Indeed time-filter strategy; no annual salary conversion; NaN handling; subprocess timeout/output limits; failures/empty uncertainty retained | Optional package is not activated or verified in this build environment. Install a separately reviewed, pinned compatible JobSpy version before setting JOBSPY_ENABLED=true. Adapter results always require independent complete-JD/eligibility verification; no result is silently accepted. LinkedIn is forbidden. |
...
Executed scoped connector verification: **48 source-contract tests and 23 SafeHTTP tests passed**; ruff check passed. The source-contract tests were last included in the 82-test security/configuration/HTTP run (three upstream dependency deprecation warnings). Tests cover successful source contracts, real replay, unsafe URLs/private and metadata addresses, mixed DNS answers, DNS pinning, redirects and secret stripping, byte limit, backoff, closed-vs-blocked pages, preserved qualifications, last-publication semantics, date uncertainty, and explicit optional configuration. Network tests are not silently run with ordinary pytest. Database ingestion/worker/report checks are tracked in BUILD_STATUS and TEST_RESULTS.
