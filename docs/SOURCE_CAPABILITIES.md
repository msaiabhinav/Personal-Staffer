...
| amazon.jobs | Public `search.json` feed with complete postings per hit; date-only publication; listing-based opening verification; country/state/arrangement from the structured locations | Registered 13 September 2026 (tenant `USA`, four reviewed queries). Real runs persisted 63+ postings per query; the first postings that passed every rule came from this source. Per-id lookups are used for fetch and verification. |
| Eightfold (Microsoft) | `pcsx/search` JSON listing with `location` filter; `apply/v2/jobs/{id}` JSON detail with exact creation time, locations, work-location option; detail-based opening verification | Registered 13 September 2026 from the owner's URL (`apply.careers.microsoft.com`, domain `microsoft.com`, location United States). Real run persisted postings with complete descriptions. Employment type is not in the detail. |
| Talemetry (HCA Healthcare) | `jobs/search?q=` JSON listing (Accept: application/json); job page JSON-LD JobPosting normalised by the direct reader; identity by page URL | Connector tested against recorded payloads, but the live site is behind a Cloudflare bot challenge (`cf-mitigated: challenge`, HTTP 403) for the app's HTTP client while a plain curl passes — i.e. TLS-fingerprint bot detection. The product does not impersonate browsers, so the HCA source is registered but **disabled/BLOCKED**. |
| Radancy (Intuit) | `search-jobs/results` endpoint (HTML fragment inside JSON) parsed for job links, titles, locations and total; job page JSON-LD; non-padded `datePosted` zero-padded before the date rule | Registered 13 September 2026 from the owner's URL (`orgIds=27595`, location United States). |
...
## Owner-supplied career sites that cannot be scanned (13 September 2026)

- Tesla (`tesla.com/careers`): every request, including the site's own JSON state endpoint and a browser-like user agent, is answered "Access Denied" by the edge (Akamai). Not scannable without bypassing bot detection, which the product does not do.
- Walmart (`careers.walmart.com`): a Next.js app whose only job search is an AI "hybrid-search" POST returning compressed, undocumented payloads (405/520 to plain requests). Not implemented.
- HCA Healthcare (`careers.hcahealthcare.com`): Cloudflare challenge for non-browser TLS clients (see the Talemetry row). Registered, disabled.
- All remain watchable; JobSpy (Indeed/Glassdoor aggregation) is the documented alternative once its dependency audit clears (JOBSPY_SETUP.md).
