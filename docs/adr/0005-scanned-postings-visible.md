...
- `relevant_only=true` restricts the scope to postings whose latest evaluation's `role_relevance` rule is not `ROLE_UNRELATED`; never-evaluated postings are excluded rather than assumed relevant. The company page and the feed preview use it (owner: "the jobs shown in the watchlist must be relevant").
- The feed gains a "Scanned postings (preview)" scope capped at 5 newest relevant postings, clearly marked as a preview.
