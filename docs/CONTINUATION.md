...
4. Done: Google sign-in works in live-local mode; nine real sources registered and scanning; E-Verify informational (ADR 0003); Gmail connected (299 messages, 150 days); one-time application import and status re-match applied (68 active applications, 12 rejected, 1 interviewing); notification bulk actions and email alerts (ADR 0004); watchlist company pages and scanned-posting preview (ADR 0005). Application dashboard rebuilt as a real overview (KPIs, pipeline, attention, weekly bars, activity, waiting-longest). Sources now 12 (Henry Ford Health via SmartRecruiters, Thermo Fisher via Workday, Amazon via the new amazon.jobs connector; scheduled per reviewed query, hourly). Company search/page on the dashboard, clickable KPIs, owner's logo as icon/theme (ADR 0002 note), worker priorities and a 10-minute reconcile window (the 2-minute window flooded Redis with 2,672 duplicate messages). Gmail capture widened (ADR 0006) and a re-backfill is in progress; when `gmail_sync_state.history_cursor` is set again run `import-email-applications` (preview, then `--apply`) and `rematch-email-reviews --apply`, then record the derivation in TEST_RESULTS. Next: rebuild the Windows client after any client change (the running debug exe is launched directly from build\windowsdunner\Debug; a desktop shortcut points there) and let the owner clear the 299 imported review alerts with Mark all read / Delete; watch the first daily report at 11:00 America/New_York; review withheld counts per source in Settings > Source health; consider per-type push toggles if the owner wants fewer kinds; register career-site sources for the watched companies that have none (11 of 12 — most use Phenom/custom sites, so a new connector or JobSpy qualification is needed); Ashby country evidence (Mercury shows COUNTRY_NOT_ESTABLISHED). The owner intends to redesign the UI.
