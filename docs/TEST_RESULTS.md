...
|---|---|---|
| Tray enable + close-to-tray | Works: closing the window keeps the process alive; `HKCU\Software\PersonalStaffer\TrayEnabled=1`. The owner initially saw no icon because **Windows 11 places new notification-area icons under the "^ hidden icons" flyout**; UI Automation enumeration of that flyout lists "Personal Staffer — background notifications". | Settings description now says where the icon lives; enabling tray mode shows a one-time balloon from the icon (`flutter_window.cpp`). Tray Open/Exit menu still to be confirmed by the owner from the flyout. |
