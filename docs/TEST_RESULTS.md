...
### First real source-to-PostgreSQL run on the laptop (live-local mode) — 12 September 2026

Registered `greenhouse/oscar` (Oscar Health) and `workday` (`https://analogdevices.wd1.myworkdayjobs.com/External`, watchlisted Analog Devices) with `python -m app.cli register-source`; `seed-registry` created the eleven priority watchlist entries for the owner. `run-search --source <oscar>`: **288 discovered, 288 fetched with complete descriptions, 288 persisted**, state PARTIAL (opening verification unresolved for every posting), 276 s through the production DNS-pinned SafeHTTP client — the first live pipeline evidence on production transport. The scheduler then ran its own SEARCH_SOURCE work items for both sources (327 distinct jobs).

Rule outcomes across the run (per rule, 1,470 evaluations at the time): active_opening UNKNOWN 100 %; everify UNKNOWN 100 % (no legal-entity evidence yet); role_relevance PASS 65 / FAIL 1,395 / REVIEW 10; freshness PASS 105 / FAIL 1,335 / UNKNOWN 30; country PASS 24 / UNKNOWN 1,446; employment PASS 1,420 / FAIL 50; experience PASS 465 / FAIL 793 / REVIEW 212; sponsorship PASS 1,455 / REVIEW 15. Every job was withheld (INELIGIBLE/NEEDS_REVIEW) as policy requires without E-Verify and opening evidence; no job was delivered.

Defect found and fixed: each rescan of an unchanged posting appended a new snapshot and a full evaluation (1,517 evaluations for 327 jobs within six minutes; unbounded growth at the 15-minute watchlist cadence). `ingest_normalized` now returns the retained evaluation when the source record, description hash/completeness, normalized fields (fetch time excluded) and opening status are unchanged, and only touches `last_seen`; changed content still creates an immutable snapshot and evaluation. Regression: `test_rescan_of_unchanged_postings_appends_no_snapshot_or_evaluation` (real PostgreSQL). Suite: **460 passed, 0 skipped**. `test_optional_config_states_and_no_synthetic_production` now clears integration variables so it also passes inside a live-local container.

### Live pipeline follow-ups after the first real run (12 September 2026)

- **Opening verification**: Oscar Health's career pages render Greenhouse postings client-side, so the generic HTML check saw neither the title nor an Apply control (all 288 `OPENING_NOT_VERIFIED`, HTTP 200). `GreenhouseConnector.verify_opening` now uses the Job Board API (job id + application questions + `absolute_url` = ACTIVE; 404/410 = CLOSED); `AshbyConnector.verify_opening` uses the posting API listing. Re-run: `opening_active` 288/288, run state SUCCEEDED.
- **Country evidence**: Greenhouse never set `country_codes`, so 308 of 328 postings were `COUNTRY_NOT_ESTABLISHED` despite locations such as "New York, New York, United States". `country_from_location()` reads explicit country tokens only (never a two-letter part that could be a state; bare "Remote" stays unknown) and is used by Greenhouse and as a Workday fallback.
- **E-Verify** (ADR 0003): owner decision; `EVERIFY_GATE=INFORMATIONAL` in `.env.local`. The rule no longer withholds; the fact stays honest (`EVERIFY_NOT_CONFIRMED_INFORMATIONAL`, flag `EVERIFY_INFORMATIONAL`). The suite pins `REQUIRED`.
- After all three, Oscar Health's board still yields **0 eligible**: of 288 open postings only 21 were published within 72 h and none of those is an analyst-family role that also meets the experience rule — an honest low count, not a failure. Remaining non-pass rules on fresh postings were role relevance, experience (`4+`/senior) and one required clearance.
- Nine live sources are now registered for the owner (Oscar Health, Analog Devices/Workday, Anthropic, Mercury, Vercel, Flatiron Health, Cockroach Labs, Ramp, Notion); the scheduler rescans them on its own cadence.
- Suite after these changes: **466 passed, 0 skipped** on PostgreSQL/Redis (new tests: Greenhouse/Ashby verification, location country parsing, informational E-Verify, unchanged-rescan short-circuit, config isolation).

### Gmail history import, status re-match and notification actions (12 September 2026)

- One-time import (owner request, 150-day window, 299 messages): 100 confirmation emails proposed; `import-email-applications --apply` created 80 applications and linked 7, skipped 13 low-confidence rows. 12 mis-imports (e.g. "Business Analyst", "Tsenta", "Application Complete") were voided with `void-imported-applications` (UNDO_APPLIED, 13 reviews reopened) → **68 active applications**.
- `rematch-email-reviews --apply` linked **31** open status emails to the single application at the named company: 13 status events (12 REJECTED, 1 INTERVIEWING — Johnson & Johnson's interview invitation followed a "not selected" mail for the same requisition; the later event wins). 168 `UNMATCHED` reviews remain open for the owner (multi-application companies, newsletters, unnamed employers).
...
### Watchlist company pages and scanned-posting preview (13 September 2026)

- ADR 0005: `GET /jobs?scope=scanned[&employer_group_id=]` lists open postings collected by enabled sources without delivering them; `_accessible_job` accepts collected postings; `GET /watchlist/{id}` (sources, open postings, delivered) and `source_count`/`open_postings` on the list.
- Client: Watchlist rows show "N open posting(s)" or "No career site registered" and open a company page listing postings with a Qualifies / Withheld · reason / Not evaluated pill; Job Feed scope "Scanned postings (preview)" shows the 5 newest with the same pill.
- Live check: preview returns Cockroach Labs, Mercury and Oscar Health postings, all withheld with reasons (experience above policy, country/employment not established). Watchlist: Analog Devices 1 source / 40 open postings; the other eleven companies have no registered career site (Intuit still PENDING identity).
- Follow-up noticed: Ashby postings (Mercury) carry `COUNTRY_NOT_ESTABLISHED`; the Ashby connector should derive `country_codes` from location like Greenhouse/Workday now do.
- Suite: backend **490 passed, 0 skipped** (new: scanned scope lists undelivered postings, hides closed ones and disabled sources, creates no delivery; watchlist entry detail); Flutter analyzer clean, **25 tests**.
- Owner follow-up ("the jobs shown in the watchlist must be relevant"): `relevant_only=true` keeps postings whose latest `role_relevance` rule is not `ROLE_UNRELATED` (never-evaluated postings are excluded); the company page and the feed preview both use it, and cards carry a "Relevant · <family>" / "Title matches · responsibilities unclear" pill beside the verdict. Live: the preview shows Oscar Health "Business Operations Senior Manager" and Notion FDE postings; Analog Devices lists one relevant posting ("Senior Principal Forward Deployed Engineer" via the profile's "Deployment" family).
- Career-site systems behind the watched companies without a source (probed 13 Sep): Thermo Fisher = Phenom; Intuit = Radancy; Henry Ford Health = SmartRecruiters (public postings API); Amazon = amazon.jobs JSON search (public); Walmart = custom Next.js site; HCA, Cognizant, Tesla, Infosys returned 403 to a plain client; Microsoft/TCS endpoints did not answer. None is covered by the Greenhouse/Ashby/Workday connectors — new connectors are required.

### Gmail capture re-run and honest numbers (13 September 2026, overnight)

Derivation for the owner ("you didn't show accurate numbers"):

- Gmail search (150 days, wider query per ADR 0006): **435 messages listed** by Gmail. Of these the parser stored **376** as job-related (was 299 with the old query); 59 were classified `UNRELATED` (marketing/job-alert mail with no candidacy wording) and are not stored. The second pass followed a parser change: mail saying *applied / applying / candidacy / candidate / resume / next steps* is now kept for review instead of dropped (38 more messages).
- Classification of the 376: 100 application confirmations, 25 rejections, 6 status updates, 5 information requests, 2 position-closed, 1 interview, 1 offer, 2 forwarded, 2 ambiguous, 137 unrecognised employer templates, 95 with partial content.
- Applications: the 100 confirmations yielded **68 active applications** (80 created + 7 linked − 12 voided mis-imports − placeholders merged); 26 low-confidence confirmation reviews remain for the owner (no readable company/role).
- Statuses from employer mail: **14 rejected**, 1 interviewing (Johnson & Johnson), 53 with no reply yet. Re-match linked 31 + 3 emails (Personify Health and The Home Depot rejections added this pass); newsletters and identity prompts are now excluded from re-match (`_BULK_MAIL`).
